...
- **One-way platform — 2 bug CRITICAL/thật bắt được qua review đối kháng (workflow 3 lens: correctness/determinism-safety/test-gap), đã tự verify bằng cách chạy engine thật, không phải đoán:** (1) so sánh `crosses` dùng `<` chặt khiến người đứng yên trên platform rơi xuyên ngay sau đúng 1 tick không bấm gì (vì lúc đáp `y` đã bằng chính xác `platform.y`, tick sau không còn "nhỏ hơn thật" — sửa bằng `<=`); (2) drop-through dùng bộ đếm tick chung sẽ vô tình xuyên qua CẢ platform khác không liên quan nếu nằm gần platform đang xuyên (sửa bằng cách chỉ loại trừ đúng platform đang xuyên qua, lưu theo tham chiếu `Platform`, không phải bộ đếm thời gian). Cả hai bug KHÔNG bị lộ bởi bộ test ban đầu vì test chỉ gọi `_step_vertical` 1 lần/kịch bản đơn giản — bài học: test đa-tick (đứng yên nhiều tick, 2 platform xếp tầng) quan trọng ngang test 1-tick cho mọi cơ chế trạng thái-bền (stateful) sau này. Đã thêm 2 test tương ứng, 12/12 xanh.
- `StateHasher` đã bổ sung hash `on_platform` + trạng thái drop-through (trước đó thiếu — phát hiện qua cùng đợt review) để desync liên quan platform lộ ra ngay tick đó thay vì trễ 1 tick qua y/vy.
...
## D-012 · Art pipeline — REVIEW-AT-GATE (Gate C, tại art proof) · Sửa 2026-07-20
**Quyết định (đảo hướng so với bản 2026-07-15):** Chuyển hướng ưu tiên sang **painterly HD sinh bằng AI**, dùng `agent-sprite-forge` (chạy qua Codex CLI, ngoài phiên Claude Code) làm pipeline sinh + hậu kỳ chính, thay cho pixel-art/Aseprite. Vẫn giữ **sprite-sheet frame-rời-rạc** (không chuyển sang skeletal rig kiểu Spine/DragonBones) để khớp thẳng animation contract hiện có (`frameCount/fps/events`) — lý do người dùng chốt: "game đầu tiên sprite sheet sẽ dễ hơn" (né rig xương, kỹ năng chuyên biệt chưa có). Sản xuất thật vẫn đợi qua fun-gate M2 (không đổi D-010); test hiện tại là art proof cho chính Gate C này.

**Art proof — nhân vật "Nori" (raccoon), 3 state đã test, số liệu từ `pipeline-meta.json` từng bộ:**
- `idle` (4 frame, `art/tests/agent-sprite-forge/nori/idle/processed/`): `body_scale_cv=0.0233` (ngưỡng 0.08), `anchor_y_std=0.0143` (ngưỡng 0.05), `edge_touch_count=0`. Pose gần như đứng yên (chớp mắt/vẫy tai) — chưa phải phép thử khó.
- `attack` bản 1 (6 frame, lưới 3×2 hình chữ nhật hẹp, `.../attack/processed/`): `body_scale_cv=0.0517` (qua nhưng gấp đôi idle), `profile_body_scale_drift=0.0897`/ngưỡng `0.1` (sát nút 90%), **`edge_touch_count=2`** — 2 frame (attack-1, attack-2) bị cắt xén thật vì tư thế (tay/súng vươn) chạm mép ô lưới hẹp. Đây là lỗi thật, không phải suy đoán.
- `attack-v2` (9 frame, đổi lưới 3×3 vuông rộng hơn, `.../attack-v2/processed/`): sau khi sửa layout, mọi số liệu cải thiện rõ — `body_scale_cv=0.0415`, `profile_body_scale_drift=0.0491` (còn nửa ngưỡng), `edge_touch_count=0`, `component_count=1` mọi frame (hết vỡ mảnh nền). Frame attack-8 có tư thế thật sự khác (cúi người, tay xoè kiểu giật lùi) chứ không lặp lại pose idle — vẫn giữ đúng màu áo/tỉ lệ/mặt nhân vật. **Đây là phép thử pose-khác-hẳn-nhau đã PASS.**
- `hurt` (4 frame, ngưỡng QC nới hơn theo đúng thiết kế tool cho state này: `max_body_scale_cv=0.12`, `max_profile_scale_drift=0.18`): `body_scale_cv=0.0343`, `profile_body_scale_drift=0.00718` (rất thấp), `edge_touch_count=0`. Pose loạng choạng/tay giơ lên rõ khác idle, vẫn giữ đặc điểm nhân vật.

**Bài học vận hành (giữ lại để không lặp lỗi):** lưới ô hình chữ nhật hẹp (3×2) làm AI vẽ tư thế động bị cắt xén ở mép — đổi sang lưới ô vuông đủ rộng (3×3, hoặc canvas lớn hơn cho ít frame/ô hơn) là fix đã kiểm chứng. Áp dụng ngay từ đầu cho mọi state động (attack/hurt/ko/launch), không đợi phát hiện lại.

**Rủi ro còn treo — chưa đạt gate hoàn toàn:** walk/run cycle (di chuyển chân lặp lại, lịch sử hay là state khó giữ nhất quán nhất) và ko/launch/tumble (theo bảng frame budget ở lịch sử D-012 cũ) chưa test. Vấn đề vỡ mảnh nền (`component_count` bất thường) không còn xuất hiện ở attack-v2/hurt nên hạ mức độ quan tâm, nhưng chưa coi là loại trừ hẳn — soát mắt khi có pose vũ khí vung xa thân hơn nữa.

**Lý do đảo hướng so với bản trước:** Bản 2026-07-15 loại painted/rig vì "khó đạt hơn pixel với người mới, không khớp animation contract frame-rời-rạc". AI-gen qua `agent-sprite-forge` + giữ nguyên sprite-sheet giải quyết đúng 2 điểm đó: không cần kỹ năng illustration tay (AI model vốn mạnh painterly hơn pixel-art thật), và không đổi cấu trúc animation sang rig.

**Điều kiện đảo lại (rollback):** Nếu walk cycle hoặc ko/launch/tumble không đạt cùng ngưỡng QC trên (kể cả sau khi áp bài học lưới-vuông), hoặc nhân vật không giữ được đặc điểm ổn định qua nhiều lần generate độc lập, quay lại pixel-art/Aseprite (Pha A/B, xem lịch sử git bản 2026-07-15) làm phương án dự phòng.
